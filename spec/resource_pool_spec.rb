...
describe ResourcePool do
  ...
  it "should reserve and free" do
    ...
    @pool.reserve(id).should eq(nil)
    lambda { @pool.free(123, "a") }.should raise_error("Can't free resource a. It's reserved for 'myid' and 123 tried to free it!")
    @pool.reserve(id).should eq(nil)
    @pool.free(id, "a")
    @pool.reserve(id).should eq("a")
  end
  ...
  it "should claim, reserve and free" do
    id = "myid"
    id2 = "myid2"
    @pool.claim_for_other(id, 10).should eq(nil)
    @pool.edit_data do |pool|
      pool.cached_data= {"a" => "1"}
    end
    lambda { @pool.reserve_claim(id2, "a:qweqe") }.should raise_error("Resource a not claimed!")
    claim_key = @pool.claim_for_other(id, 10)
    arr = claim_key.split(":")
    arr[0].should eq("a")
    claim_id = arr[1]
    arr.size.should eq(2)
    @pool.reserve(id).should eq(nil)
    lambda { @pool.free(123, "a") }.should raise_error("Can't free resource a. It's claimed for 'myid'!")
    @pool.reserve(id).should eq(nil)
  ...
  it "should allow claimed to be released by the creator" do
    id = "myid"
    @pool.edit_data do |pool|
      pool.cached_data= {"a" => "1"}
    end
    claim_key = @pool.claim_for_other(id, 10)
    @pool.reserve(id).should eq(nil)
    lambda { @pool.free(123, claim_key) }.should raise_error("Can't free resource a. It's claimed for 'myid'!")
    @pool.free(id, claim_key)
    @pool.reserve(id).should eq("a")
  end
end